...
//        Mat Depthshow(Size(640,480), CV_8UC3,(void*)depth_frame.get_data(),Mat::AUTO_STEP);
        Depthmate = depth_mat;
    ...
    for (size_t i = 0; i < outs.size(); ++i) {
        ...
        for (int j = 0; j < outs[i].rows; ++j, data += outs[i].cols) {
            ...
            if (confidence > 0.5) {
                ...
                float k = 1.62*width;
                float  g = 1.5*height;
                float kc =fabs( 84.3 - k);
                float gc = fabs(165 - g);
                float kj = 1 - kc/84.3;
                float gj = 1 - gc/165;
                cout << "宽度为：" <<  k << "高度为：" << g << endl;
                cout << "宽度精度为:"  << 100*kj << "%  高度精度为: " << 100*gj << "%" << endl; 
